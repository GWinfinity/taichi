...
FlagosDevice::FlagosDevice() : context_(std::make_unique<FlagosContextWrapper>()) {
...
void FlagosDevice::memcpy_internal(DevicePtr dst, DevicePtr src, uint64_t size) {
  void *dst_ptr =
...
std::vector<std::string> FlagosDevice::get_supported_chips() {
  ...
  return {"mlu370", "mlu590", "ascend910", "ascend310", "dcu", "gcu", "generic"};
}
